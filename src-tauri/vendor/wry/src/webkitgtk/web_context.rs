      ...
      if let Some(uri) = request.uri() {
        ...
        let responder: Box<dyn FnOnce(HttpResponse<Cow<'static, [u8]>>)> =
          Box::new(move |http_response| {
            // VOLOPH PATCH: deliver the response at DEFAULT priority, not the
            // DEFAULT_IDLE that `MainContext::invoke` uses. An idle source is
            // only dispatched when no higher-priority source is ready, and while
            // a GtkGLArea animates every frame (embedded libmpv playback) the
            // main loop never goes idle — every custom-protocol (Tauri IPC)
            // response is starved until rendering pauses. DEFAULT is the same
            // band as the event channels that demonstrably keep flowing.
            MainContext::default().invoke_with_priority(glib::Priority::DEFAULT, move || {
