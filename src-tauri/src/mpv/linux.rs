//! Linux surface backend: mpv's OpenGL render API drawing into a `GtkGLArea`
//! overlaid on the Tauri webview (ADR 0008).
//!
...
//! (The Windows backend has no Wayland constraint and embeds via `--wid`
//! instead — ADR 0014.)
//!
//! ## Threads
//!
//! Everything touching GTK (showing, moving and rendering the `GtkGLArea`) must
//! run on the GTK main thread, so it is funnelled through a glib channel
//! ([`SurfaceTx`]) whose receiver is attached to the main loop in [`init`].
//! mpv's render-update callback (which fires from mpv's own thread when a new
//! frame is ready) posts a `Render` message down the same channel, which calls
//! `queue_render` on the main thread.
...
use super::{mpv_create, mpv_err, mpv_initialize, set_option, MpvHandle};

// ---------------------------------------------------------------------------
// libmpv render API FFI (render_gl.h) — only used by this backend.
// ---------------------------------------------------------------------------
...
// creating the handle. `LC_NUMERIC` is 1 on glibc. (The Windows backend skips
// this: nothing there touches the CRT locale, which starts as "C".)
...
impl SurfaceTx {
    ...
    pub fn rect(&self, x: i32, y: i32, w: i32, h: i32) {
        self.send(SurfaceMsg::Rect { x, y, w, h });
    }

    pub fn show(&self) {
        self.send(SurfaceMsg::Show);
    }

    pub fn hide(&self) {
        self.send(SurfaceMsg::Hide);
    }
}
...
/// Create the mpv handle and its GTK surface; must run on the GTK main thread
/// (Tauri's `setup` does). Returns the configured, initialized handle for the
