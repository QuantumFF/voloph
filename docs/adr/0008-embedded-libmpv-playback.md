...
- **Render-to-texture / zero-copy DMA-BUF into the DOM** — per-frame cost, and the webview-compositing path is already unreliable here (ADR 0010 notes video composites in a separate layer where canvas capture comes out black; the DMA-BUF renderer was disabled on NVIDIA).
